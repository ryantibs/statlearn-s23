...
\smash{$\hbeta = (X^\T X/n)^+ X^\T Y/n$}, and helpful to record the bias and
...
B_X(\hbeta) \asto \frac{r^2}{\gamma} \frac{1}{v_F(0)},  
\end{equation}
...
V_X(\hbeta) \asto \sigma^2 \bigg(\frac{v'_F(0)}{v_F(0)^2} - 1 \bigg),
\end{equation}
...
details; for the remainder, a supplementary note can be found on the course 
website.   
...
\big| B_X(\hbeta_\lambda; \beta_0) - (1+c_n) \beta_0^\T \Sigma (b_n \Sigma + 
...
estimator \smash{$\hbeta = (X^\T X)^+ X^\T Y$}, 
\begin{equation}
\label{eq:ridgeless_bias_limit}
\big| B_X(\hbeta; \beta_0) - (1+\tilde{c}_n) \beta_0^\T \Sigma (\tilde{b}_n

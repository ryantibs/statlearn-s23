...
disturbing. On the other hand, these results are not too unexpected, at least in
hindsight, and their proofs are simple, as we will see in the following
subsections.   
...
result is due to \citet{ranjan2010combining}.  
...
The fact that any linear combination of probabilistically calibrated forecasters
must be miscalibrated and systematically overdispersed (part c) is simple but
striking. In the binary case, where $Y \in \{0,1\}$, the same is true when we
replace the notion of probabilistic calibration with conditional calibration
(recalling the equivalence between the two), which reproduces an earlier result
from \citet{ranjan2010combining}.  
...
\item In particular, if $F_j$, $j=1,\dots,p$ are each marginally calibrated, as
  in \eqref{eq:prob_calib}, then $F$ is underdispersed, meaning 
  that $\Var(F^*(Y)) < \Var(U)$, where $U \sim \mathrm{Unif}(0,1)$.  
\end{enumerate}
\end{theorem}

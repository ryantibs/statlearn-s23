...
example, $V(Z_i; Z)$ when denotes a score assigned to $Z_i$ is based off 
training a model on $Z$ that uses the \emph{order} of the points in $Z$, such as
an autoregressive model for forecasting in a time series problem---then it is
still reasonable to ask what guarantees a weighted conformal prediction set
would have. However, this lies outside of the scope of the previous result given
in Theorem \ref{thm:weighted_conf_nex}. 

It turns out that we can still accomodate this case, and in fact accomodate a 
completely arbitrary score function $V$, by injecting a random swap into the
...
follows: let \smash{$Z^{(x,y)} = (Z_1,\dots,Z_n,(x,y))$}, let
...
\end{aligned}
\end{equation}
where finally we draw 
\[
K \sim \sum_{i=1}^{n+1} \tw_i \, \delta_i,
\]
independent of everything else. Then with \emph{no} assumptions on $V$ and
\emph{no} assumptions on the joint distribution of $Z_i=(X_i,Y_i)$,
$i=1,\dots,n+1$, the set \smash{$\hC^w_n$} satisfies the exact same guarantee as
in \eqref{eq:weighted_cov_nex}. 
\end{theorem}

In words, the scores \eqref{eq:scores_swap} used in the construction of the
conformal set \eqref{eq:weighted_conf_swap} are computed after swapping the 
order of a random data point with the query test point $(x,y)$. 

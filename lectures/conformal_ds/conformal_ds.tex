...
\bigg\},   
\end{equation}
where \smash{$\lceil 1-\alpha \rceil_w = \min\{ \tau \in \mathrm{range}(F^w) :
  \tau \geq 1-\alpha\}$} and we write $F^w$ for the (random) CDF of the
discrete distribution \smash{$\sum_{i=1}^{n+1} p^w_i( Z_1,\dots,Z_n,(x,y))  
  \delta_{R^{(x,y)}_i}$}. This is more complicated in the current weighted
setting (compared to what we saw for ordinary unweighted conformal methods, in 
the last lecture) because we need to adjust the nominal level of $1-\alpha$
upwards so that it lies in the range of the CDF of the weighted score
distribution; but this distribution is random, and so the adjustment is itself
random. This is the primary reason that we began with the quantile form in 
\eqref{eq:weighted_conf}, since we can always use the unadjusted level
$1-\alpha$ and completely any such complications.     
...
\begin{multline*}
\hC_n^{w,*}(x) = \bigg\{ y : \sum_{i=1}^{n+1} p^w_i\big( Z_1,\dots,Z_n,(x,y)
...
where now \smash{$\lfloor 1-\alpha \rfloor_w = \max\{ \tau \in
  \mathrm{range}(F^w) : \tau \leq 1-\alpha\}$} with $F^w$ denoting the (random)
CDF of the discrete distribution \smash{$\sum_{i=1}^{n+1} p^w_i(
  Z_1,\dots,Z_n,(x,y)) \delta_{R^{(x,y)}_i}$} as before, and 
\[
B^w \sim \mathrm{Bernoulli}\bigg( \frac{1-\alpha - \lfloor 1-\alpha \rfloor_w}
{\lceil 1-\alpha \rceil_w - \lfloor 1-\alpha \rfloor_w} \bigg).
\]
Interestingly, the randomized conformal set in the second-to-last display is
actually less intuitive---we must introduce level adjustments that were not
needed in the unrandomized quantile-based set in \eqref{eq:weighted_conf}. 
Ultimately, either version of the randomized weighted conformal set
...
\eqref{eq:weighted_cov_gen}, which is interesting.  Jokes about intractability
aside, this may be useful (and computable) in certain structured-X settings, for
example, where the sequence $X_1,\dots,X_{n+1}$ has some kind of Markov
structure.
...
we'll see next, is also very different; in a sense, it is more general in scope.
We'll only cover this material at a relatively high level, and won't give proof
details. 
...
\begin{itemize}
\item Denote $Z = (Z_1,\dots,Z_{n+1})$ the data vector (an ordered
  sequence). 
...
arguments---which is the typical assumption in conformal prediction---implies 
that we have $V(Z_j; Z) = V(Z_j; Z_\sigma)$, for any permutation $\sigma$, where  
\smash{$Z_\sigma = (Z_{\sigma(1)}, \dots, Z_{\sigma(n+1)})$}. Thus under this 
...
as follows: 
\begin{equation}
\label{eq:weighted_conf_cdf_nex}
\hC^w_n(x) = \bigg\{ y : \sum_{i=1}^{n+1} \tw_i \, 1\Big\{ R_i^{(x,y)} \leq
R^{(x,y)}_{n+1} \Big\} \leq \lceil 1-\alpha \rceil_w \bigg\},
\end{equation}
where \smash{$\lceil \beta \rceil_w = \min\{ \tau \in
  \mathrm{range}(F^w) : \tau \geq \beta\}$} and $F^w$ is the CDF of

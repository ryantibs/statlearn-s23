...
topics from which we can ``pick and choose'' for a lecture like this one. We've
chosen two such topics---and shamelessly, for each, you'll notice an overlap 
between the authors list and the author of these lecture notes. There is plenty
of other interesting work out there that we can't cover in just one lecture. 
...
becomes:      
\[
\hC^{w,*}_n(x) = \bigg\{ y : R_{n+1}^{(x,y)} \leq \mathrm{RandQuant}\bigg( 
1-\alpha; \, \sum_{i=1}^n p^w_i\big( Z_1,\dots,Z_n,(x,y) \big)
\delta_{R_i^{(x,y)}} + p^w_{n+1}\big( Z_1,\dots,Z_n,(x,y) \big) \delta_\infty
\bigg) \bigg\},    
\]
...
conformal set \smash{$\hC^{w,*}_n$} defined above then satisfies 
\[
...
orange) of the likelihood ratio weight function \smash{$w=d\tP_X/dP_X$}. We can
see from the middle left panel that its coverage is restored compared to the
naive application of conformal in the covariate shift problem, from the top
row. However, we also see that the dispersion in the coverage histogram from
weighted conformal (over the 5000 repetitions of the experiment) is larger than
that of usual conformal without covariate shift (in red), from the top row. This
is because, with non-uniform weights due to covariate shift, we are effectively
operating at a lower sample size. The middle row thus also displays the results
(in purple) of usual conformal prediction in a problem without covariate shift
but at the same effective sample size, defined as
\[
...
(even more difficult than a matrix permanent, since $\lambda$ could in principle 
...
as we'll see, is also very different; in a sense, it is much more general in
scope.  
...
\]
Consider a weighted conformal prediction set from training data $(X_i,Y_i)$,
$i=1,\dots,n$, defined as follows:
\begin{equation}
...
\citet{barber2022conformal} prove the following result. For \emph{any} joint
distribution of $Z_i=(X_i,Y_i)$, $i=1,\dots,n+1$, and any fixed weights $w_i \in
[0,1]$, $i=1,\dots,n$ the set \smash{$\hC^w_n$} satisfies
\begin{equation}
\label{eq:weighted_cov_nex}
\P\big( Y_{n+1} \in \hC^w_n(X_{n+1}) \big) \geq 1-\alpha - \sum_{i=1}^n 
\tw_i \cdot \dtv(R(Z), R(Z^i)),
\end{equation}
where $\dtv(A,B)$ denotes the total variation distance between (the
distributions of) two random variables $A,B$. To understand this result, we
still need a few more pieces of notation: 
\begin{itemize}
\item we denote $Z = (Z_1,\dots,Z_{n+1})$ and denote by $Z^i$ this vector after 
  swapping components $i$ and $n+1$; 
\item we denote by $R(Z)$ the vector of scores, that has components $R(Z)_j =
  V(Z_j; Z)$; and we denote by $R(Z^i)$ the vector of scores had the data
  sequence been $Z^i$, that has components \smash{$R(Z^i)_j = V(Z^i_j; Z^i)$}.  
\end{itemize}
Thus we can interpret the result in \eqref{eq:weighted_cov_nex} as follows. 

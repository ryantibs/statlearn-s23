...
parameters \smash{$\halpha_x,\hbeta_{x,1},\dots,\hbeta_{x,k}$} solve
\begin{equation}
\label{eq:local_poly}
...
where now $b(x) = (1,x,\dots,x^k)$, $B$ is an $n \times (k+1)$, $B \in \R^{n 
  \times (d+1)}$ is the matrix whose $i\th$ row is $b(x_i)$, and $\Omega \in
\R^{n \times n}$ is the same diagonal matrix as before. Hence again, local
polynomial regression is a linear smoother. 

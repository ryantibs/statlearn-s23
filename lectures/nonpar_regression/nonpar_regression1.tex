...
cubic spline \smash{$\bar{f}$} with knots at $t_1,\dots,t_N$ (to be defined
...
The goal is to expose you to a variety of methods over our lectures on
nonparametric regression, and give you a flavor of some interesting results, 
...
\citet{hastie2009elements} as a general reference on methods; and
\citet{vandegeer2000empirical, wainwright2019high} for analysis techniques and
probabilistic tools. 
...
building blocks for a high-dimensional nonparametric estimator. We may study 
this later, if and when we end up talking about additive models.
...
spherical kernel and bandwidth \smash{$h \asymp n^{-1/(2+d)}$} satisfies    
\begin{equation}
...
domain $U \subseteq \R^d$, we can now define the H{\"o}lder class  
\[
...
% homework already has them prove the statement for [0,1]^d), we can just leave
% it as is. And, even if we wanted to precise it's fine, as we could just define
% the Lipschitz condition (in the Holder space definition) with respect to the

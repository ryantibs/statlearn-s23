...
A note on computation: the coefficients \smash{$\hbeta = (N^\T N + \lambda
  \Omega)^{-1} N^\T Y$} can be computed in $O(n)$ operations. For this we form 
$N$ using the B-spline basis (for natural splines), as then the matrix $N^\T N +
\Omega I$ will be banded. In fact, more specialized computations are possible by
taking advantage of the more precise structure (beyond bandedness) afforded by
splines. Altogether, in practice, smoothing spline computations are extremely
fast. 
...
\caption{\it Insights from the Reinsch form of the cubic smoothing spline, for
  an example with $n=50$. The left plot shows the bottom 7 eigenvectors of $Q$,
  drawn using a color gradient: purple means smallest eigenvalue and light blue
  means largest eigenvalue. We can see that smoother eigenvectors correspond to
  lower eigenvalues. The right plot shows the shrinkage weights
  $w_j=1/(1+\lambda \sigma_j)$, $j=1,\dots,n$ for 8 values of $\lambda$, again
  drawn using a color gradient: purple means largest $\lambda$ and light blue
  means smallest $\lambda$. For $\lambda$ larger, the weights decay faster, so
  the smoothing spline estimator more strongly filters out wiggly components.}       
\label{fig:reinsch}
\end{figure}

\subsection{Equivalent kernel}

Something very interesting happens when we plot the rows of the smoothing spline
matrix $S = N(N^\T N + \lambda \Omega)^{-1} N^\T$. For evenly-spaced inputs,
they look like the translations of a kernel! See Figure \ref{fig:ss_kernel}, left
panel. Even more interestingly, for unevenly-spaced inputs, the rows still have
a kernel shape; but now, the bandwidth appears to adapt to the density of the
input points: lower density, larger bandwidth. See Figure \ref{fig:ss_kernel},
right panel.
...
\caption{\it Three sample rows of the cubic smoothing spline operator $S$
  defined over $n=100$. The left plot uses evenly-spaced inputs on a grid. These
  look precisely like translations of a kernel. The right plot uses input points
  drawn i.i.d.\ from $N(0.5, 0.01)$. These still look like kernels, but the
  bandwidth is larger in low-density regions of the input domain.}  
\label{fig:ss_kernel}
\end{figure}

What we are seeing is an empirical validation of a beautiful asymptotic result
by \citet{silverman1984spline}. It turns out that the cubic smoothing spline
estimator is asymptotically equivalent to a kernel regression estimator, with an
unusual choice of kernel. Recall that both are linear smoothers; this
equivalence is achieved by showing that under some conditions the smoothing
spline weights converge to kernel weights, under the ``Silverman kernel'': 
\[
K(t) = \frac{1}{2} \exp(-|t|/\sqrt{2}) \sin(|t|/\sqrt{2} + \pi/4), 
\]
and a local choice of bandwidth 
\[

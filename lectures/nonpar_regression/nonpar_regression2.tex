...
framework that can be used to analyze many other estimators---both nonparametric
(such as other penalized empirical risk minimizers) and parametric (such as
high-dimensional M-estimators, including the lasso).
...
\|f\|_{W^{\ell,q}(U)} \leq C \|f\|_{W^{k,p}(U)}, \quad \text{for all $f \in
  W^{k,p}(U)$}.
\]
This means that $W^{k,p}(U) \subseteq W^{\ell,q}(U)$, and further we get what
...
% Theorems 2 and 6 in Chapter 5.6 of Evans (2010); require \partial U to be
...
% Theorems 5 and 6 in Chapter 5.6 of Evans (2010); require \partial U to be
...
In the subcritical regime, the conclusion is arguably even more
interesting---you might even say, amazing! It says that knowing that derivatives
are smooth in a global (integrated) sense tells us something about local
smoothness of lower order derivatives, at each point in the domain. Given a $k$
times (weakly) differentiable function, the embedding theorem starts off with a
statement about the size of \smash{$\sum_{|\alpha| \leq k} \int_U |D^\alpha
  f(x)|^p \, dx$}, and translates this into a statement about the H{\"o}lder
constant of $D^\alpha f$ for $|\alpha| < k$.    
...
\caption{\it Illustration of the Sobolev embedding theorem. In each plot, we

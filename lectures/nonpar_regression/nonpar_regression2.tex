...
treatment of Sobolev theory. That is, we will really mostly just stick to the
details needed to understand the error analysis to come in the next section. A
lot more can be said; two excellent references on the subject are
...
sub-Gaussian random variables, with each $X_i$ having mean zero and
variance-proxy $\sigma^2$, then for any positive semidefinite matrix $Q \in
\R^{n \times n}$ and all $t > 0$,   
\[
...
that \smash{$ X^\T Q X = \sum_{i,j=1}^n Q_{ij} X_i X_j$}, and $\|Q\|_F,
\|Q\|_{\mathrm{op}}$ denote the Frobenius and operator norms of $Q$,
respectively. Note: the inequality we state here is from \citet{hsu2012tail},
which is similar to the Hanson-Wright inequality (but has simple explicit
constants).  
...
sub-Gaussian complexity bound as in Lemmas 9 or 14.

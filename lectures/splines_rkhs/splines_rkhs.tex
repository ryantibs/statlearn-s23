...
\caption{\it Illustration of the constraints for $C^1$ smoothness, for a cubic
  ...
  smoothness are drawn as a red quadrilateral. We see that the linear systems
  are entangled, since they each overlap at an anchor point.}   
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.915\textwidth]{tensor.png}
...
\]
As usual, given the solution \smash{$\hbeta$}, the tensor product regression
spline makes predictions according to  
\[
\hf(x) = \sum_{j \in [N]^d} \hbeta_j \, g_{j_1}(x_1) \otimes \cdots \otimes 
g_{j_d}(x_d).
\]
...
piecewise polynomial that is also $C^{k-1}$. A tensor product of univariate
$k\th$ degree splines produces a function that is indeed $C^{k-1}$, and indeed
a piecewise polynomial---it is a polynomial on hypercubes of the form    
\[
[t_{j_1}, t_{j_1+1}] \times \cdots \times [t_{j_d}, t_{j_d+1}]. 
\]
But it is \emph{not} necessarily a $k\th$ degree polynomial on these
hypercubes. Rather, it is a tensor product of univariate $k\th$ degree
...
\]
There is important difference here, compared to \eqref{eq:poly}. As a concrete 
example, take $k=1$ and $d=2$: then the tensor product of univariate linear
functions give us (what are called bilinear) functions of the form
\[
1 + a x_1 + b x_2 + c x_1 x_2,
\]
which is \emph{not} linear, because of the cross term $x_1 x_2$. At the same
time, we can't get any quadratic out of this, because we're missing $x_1^2$ and
$x_2^2$. 

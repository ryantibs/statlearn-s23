...
Proof for the case $k=3$: we'll follow Chapter 2.2 of
\citet{green1993nonparametric} for a nice direct proof. 
...
\minimize_f \; \sum_{i=1}^n (y_i - f(x_i))^2 + \lambda \int_U \sum_{|\alpha| =
  m} [D^\alpha f(x)]^2 \, dx,   
\end{equation}
for an integer order $m \geq 1$. Here we can take $U$ to be a subset of $\R^d$
containing the inputs $x_i$, $i=1,\dots,n$, and consider the minimization to be
over the Sobolev space $W^{m,2}(U)$; recall, this contains $m$ times weakly
differentiable functions with $\alpha\th$ derivative in $L^2(U)$, for $|\alpha|
\leq m$.  
...
functions on $U$ equipped with the $L^\infty$ norm.\footnote{On $C^0(U)$,
  as with all H{\"o}lder spaces, we take to be the $L^\infty$ the sup norm,
...
a Hilbert space $\cH$ is an RKHS if and only if all of the evaluation operators
are continuous.  

...
prescribed  values at the vertices. This basically happens because particular
coefficients are entangled in too many linear constraints. Figure
\ref{fig:c1_constraints} gives an illustration.          
...
\caption{\it Illustration of the constraints for $C^1$ smoothness, for a cubic
  on each triangle, right: quintic degree $k=5$. Credit:
  \citet{deboor2009way}. It turns out that a polynomial of an arbitrary degree
  on a triangle can be written be in what is called B-form (in an expansion of 
  Berstein basis polynomials), whose coefficients can be assigned to anchor
  points drawn as open blue circles. When $k=3$, the set of coefficients that
  are tied together in a linear equation needed for $C^1$ smoothness are drawn
  as a red quadrilateral. There are two anchor points whose coefficients are 
  simultaneously involved in three equations.}

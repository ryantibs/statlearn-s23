...
triangles sharing an edge $e$. Then on each triangle we have \smash{${4 \choose
    2} = 6$} parameters to define the polynomial. Denoting by $f,g$ the
...
D^\alpha f(x) = D^\alpha g(x), \quad \text{for all $|\alpha| \leq 1$ and all $x
  \in e$}. 
\] 
It can be shown that this reduces to \smash{${3 \choose 2} + {2 \choose 1} = 5$} 

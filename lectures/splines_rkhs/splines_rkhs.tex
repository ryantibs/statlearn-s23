...
\infty$ in \smash{$\|\cdot\|_{W^{m,2}(\R^d)}$} norm, the continuous embedding
...
in \smash{$\|\cdot\|_{W^{m,2}(\R^d)}$} norm but $f_k(x) \not\to f(x)$ for some
...
\smash{$\|\cdot\|_{W^{m,2}(\R^d)}$} norm. However, recall $f_k(0) = 1$ for all
...
$\|\cdot\|_\cH$, by \smash{$\|f\|_\cH^2 = \langle f, f, \rangle_\cH$}. Equipped
...
% \]
Meanwhile, for $d=1$ and $m=1$, we get:
\begin{equation}
\label{eq:ss_rkhs}

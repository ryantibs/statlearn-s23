...
\large Advanced Topics in Statistical Learning, Spring 2023}
\author{}
\date{}
...
\end{gather*}
The reason for the extra condition is this: if didn't have independence of each
$x_i$ and $\epsilon_i$, then after conditioning on $x_i$, $i=1,\dots,n$ in
\eqref{eq:random_x}, the distributions of $\delta_i = \epsilon_i | x_i$,
$i=1,\dots,n$ need not be identical. 
...
or $y_0$.% \footnote{Yet another convention is to call $\E[ (f(x_0) -
  % \hf(x_0))^2]$ the \emph{excess risk}, since it equals \smash{$\E[ (y_0 -
  %   \hf(x_0))^2] - \inf_g \E[ (y_0 - g(x_0))^2]$}, where the infimum is
  %   achieved at $g=f$, the regression function.}     

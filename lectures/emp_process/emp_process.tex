...
for \smash{$\lambda = \gamma n^{-2/(2+w)} J(f_0)^{2w/(2+w) - v}$}, it holds that 
\begin{equation}
\label{eq:error_bd2}
...
\eqref{eq:basic_ineq_v1}, using Cauchy-Schwarz and the above bound, we have on
$\Omega_1$, 
\begin{align*}
...
with probability at least \smash{$q_n = 1 - \exp(-c_1\gamma) - \exp(-c_4 
  n^{2m/(2m+1)})$}. For \smash{$\lambda = \gamma n^{-2m/(2m+1)}
  J_0^{-4m/(2m+1)}$}, it holds that
\begin{gather*}

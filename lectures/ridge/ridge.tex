...
\item[(A1)] $X = Z \Sigma^{1/2}$, where the entries of $Z \in \R^{n \times d}$
  are i.i.d.\ with mean zero and unit variance;
...
standard least squares estimator, which we can rewrite as      
\begin{equation}
...
\eqref{eq:model}, using similar assumptions to (A1)--(A3) for the feature 
model, but will allow $\gamma \geq 1$. To be specfic, we assume:  
\begin{itemize}
...
We will analyze the asymptotic out-of-sample risk of the ridge estimator,
conditional on $X$, as in \eqref{eq:risk}. For convenience we will
reparameterize the ridge estimator as   
\begin{equation}

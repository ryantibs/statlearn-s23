...
\smash{$\tilde Z_{ij} = \tau^{-1} Z_{ij}$} has mean zero and unit variance. If
...
and variance $\sigma^2$, and $X \in \R^{n \times d}$ has independent rows with a 
distribution specified below. 

where $X \in \R^{n \times d}$ has independent rows, $\epsilon$ has i.i.d.\
entries with mean zero and variance $\sigma^2$, and $\epsilon \indep X$. As a
base set of assumptions, we use the standard ones in random matrix theory (with
$x_i$ denoting the $i\th$ row of $X$):
\begin{itemize}
\item each $x_i = \Sigma^{1/2} z_i$, for a covariance matrix $\Sigma$ and random
  vector $z_i$ that has i.i.d.\ entries with mean zero, unit variance, and
  $8+\eta$ moments, for some $\eta>0$;    
\item the eigenvalues of $\Sigma$ are bounded away from $0$ and $\infty$; 
\item as $n,d \to \infty$, we have $d/n \to \gamma \in (0,\infty)$, and
  \smash{$F_\Sigma \dto H$}, where $H$ is a measure supported on $[0,\infty)$. 
\end{itemize}



$\epsilon \indep X$, where $X = Z \Sigma^{1/2}$ for a 
...
we will study the limiting risk of the least squares estimator \smash{$\hbeta =
  (X^\T X)^{-1} X^\T Y$}, which we can rewrite as   
\begin{equation}
...
problem \eqref{eq:ridge} after rescaling the loss term by $1/n$. As in the least
squares analysis, we will assume the linear model \eqref{eq:model}, and place
the same assumptions on $\epsilon, X, \Sigma$, and $Z$, and further assume that
the entries of $Z$ have $8+\eta$ moments, for some $\eta>0$.
...
ridge. At the outset, we will record the following facts, which you'll verify
on the homework. The bias and variance components of the risk \eqref{eq:risk} of 
...
vector in \eqref{eq:model}. Below we analyze the bias and variance each in turn.      
...
calculation of the bias tractable, we will place a spherical prior on $\beta_0$,
such that       
\begin{equation}
\label{eq:prior}
